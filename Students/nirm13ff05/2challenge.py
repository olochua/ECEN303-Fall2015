...
giveeven = ((1.0*EvenTrials.count(4))/ (1.0*len(EvenTrials)))


print( "The empirical probability that the number of flips is 4 conditional on the number of flips being even is {0:.4f}.".format(giveeven))
...
for TrialIndex2 in range(0, NumberTrials):
    ...
    count = 0
    while Final == 0:
        TryA = biasedcoinflip(ParameterP)
        TryB = biasedcoinflip(ParameterP)
        if (1.0*TryA + 1.0*TryB) == 1.0:
            Final = 1
            count += 1
            Flipscount.append(count)
            Aheadsup.append(TryA)
        else:
            count += 1
            Final = 0
